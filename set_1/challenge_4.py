...
def main():
    ...
    msgs = []
    with open("set_1/challenge_4.txt") as f:
        for line in f:
            msgs.append(line.rstrip())

    results = []
    for i, msg in enumerate(msgs):
        print(f"Decoding message: {i}")
        encoded = bytes.fromhex(msg)
        result = xor_decode(encoded)
        results.append(result)

    results.sort(key=lambda decoded: -decoded.score)
    best = results[0]
    print(f"Best message candidate: score: {best.score} msg: {best.decoded}")
