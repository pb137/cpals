...
def xor_buffer_with_byte( bytes_buf, xor_byte ):
    return bytes(a^xor_byte for a in  bytes_buf)
...
def freq_dist( test_bytes ):
    observed = [0] * 256
    val = 1.0 / len(test_bytes)
    for ind in test_bytes:
        observed[ind] += val

    return observed

def plot_freq_dist( test_bytes ):

    test_freq = freq_dist(test_bytes)

    x = np.arange(256)  # the label locations
    width = 0.35  # the width of the bars

    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, test_freq, width, label='Decoded')
    rects2 = ax.bar(x + width/2, english_letter_weights, width, label='English')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Probability')
    ax.set_title('Decoded vs English letter frequencies')
    ax.legend()
...
def main():
    ...
    # An incorrect guess
    decoded = xor_buffer_with_byte(encoded, 50)
    plot_freq_dist(decoded)

    # The correct answer 
    decoded = xor_buffer_with_byte(encoded, 88)
    plot_freq_dist(decoded)
