...
class AZeroSRPClient(SRPClient):

    def __init__(self, n, g, k, email, passwd):
        super().__init__(n, g, k, email, passwd)

    def c_to_s_msg_1(self):
        # Here client sends pubkey as zero
        return self.email, 0
    
    def set_K(self):
        # Client will know that server will 
        # calculate S as zero, hence can calculate value of K
        # that will match server without knowing password
        S = 0
        KH = hashlib.sha256()
        KH.update(S.to_bytes(self.len_n))
        self.K = KH.digest()
...
def main():
    ...
    client = AZeroSRPClient(p, 2, 3, "client@email.com", "SuperSecretPassword")
    ...
    # These will now be different
    # Value of S server uses will be 0
    # client can send
