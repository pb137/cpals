...
class SRPServer(SRPCommon):
    ...
    def __init__(self, n, g, k, email, passwd):
        ...
        # In this protocol, server does not mix in password
        # self.B = (self.B + self.k * self.v) % self.n
        self.u = int.from_bytes(secrets.token_bytes(SRPServer.u_size_bytes))
...
class MITMSRPClient(SRPServer):
    
    def __init__(self, n, g, k, email, passwd):
        super().__init__(n, g, k, email, passwd)

    def calculate_K(self, password):
        if self.A is not None:

            # Calculate value of x for selected password
            xH = hashlib.sha256()
            xH.update(f"{self.salt}{password}".encode("utf-8"))
            x = int.from_bytes(xH.digest())

            # Calculate v from x
            v = pow(self.g, x, self.n)

            # Do normal server side calculation of S 
            S = (self.A * pow(v, self.u, self.n))
            S = pow(S, self.b, self.n)
...
def normal_exchange():
    ...
    client.salt, client.B, client.u = server.s_to_c_msg_1()
...
def mitm_exchange():
    ...
    # Initialise client and server with information known to both
    server = MITMSRPClient(p, 2, 3, "client@email.com", "Unknown")
    client = SRPClient(p, 2, 3, "client@email.com", "SuperSecretPassword")
    ...
    # At this point, client can calculate HMAC(K, salt) and send to server
    # Server can then do a dictionary attack

    passwords_to_try = ["Password1", "Password2", "SuperSecretPassword"]
    for password in passwords_to_try:

        K_server = server.calculate_K(password)
        if K_server == client.K:
