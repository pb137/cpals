...
def main():
    ...
    # Show temper and untemper works
    for i in range(100):
        y = random.randint(0,1<<32)
        tempered = MersenneTwister.temper(y)
        untempered = MersenneTwister.untemper(tempered)
        print(f"Tempering: {i} {y} {tempered} {untempered}")

    # Generate a few RNGs to show clone works
    original_mt = MersenneTwister(seed=1234)
    cloned_mt = clone_mersenne_twister(original_mt)

    for i in range(100):
        print(f"{i}: original:{original_mt.random()} cloned:{cloned_mt.random()}")
